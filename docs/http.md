...
This document defines the first concrete HTTP and HTTPS transport design for `ducknng`. It is binding for future work on `ducknng_start_http_server(...)` and `ducknng_ncurl(...)`. The purpose of the document is to pin the intended SQL surface and carrier semantics now, before implementation pressure causes the project to grow a second RPC surface that drifts away from the existing manifest, session, and Arrow contracts.
...
The concrete planned low-level HTTP client helper is:
...
The request-side `headers_json` argument should use the same canonical JSON shape for symmetry. The preferred contract is an array of objects such as `[{"name":"Content-Type","value":"application/json"}]` rather than a plain JSON object, because HTTP header names may repeat and order sometimes matters operationally.

This helper is transport-local and not manifest-derived. It is meant for generic HTTP interactions, adapter debugging, and future interoperability helpers. It is not the only planned route to `ducknng` RPC over HTTP.
