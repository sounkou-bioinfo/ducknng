...
3.  **Framed RPC, registry policy, authorization, and sessions.** The
    ...
    `idle_timeout_ms`, `max_open_sessions`, and
    `max_sessions_per_peer_identity`, is exposed in service/manifest
    metadata. Live counters such as `active_pipes` and
...
- session ownership by `session_token`, additionally bound to verified
  ...
  server-owned effective session policy including `idle_timeout_ms`,
  `max_open_sessions`, and `max_sessions_per_peer_identity`;
  `ducknng_list_servers()` also exposes current `active_pipes`,
  `inflight_requests`, and configured
