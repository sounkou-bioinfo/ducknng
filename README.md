...
background-job protocol. The current server is still unauthenticated at
the method level, but query sessions are no longer
bearer-by-`session_id`: `ducknng_open_query(...)` returns a
`session_token` that must accompany `fetch`, `close`, and `cancel`.
WebSocket here means NNG’s `ws://` and `wss://` transports, not a second
HTTP-specific RPC surface.
...
```
    ...
    │ query_open │ query   │ Open a server-side query session                │ reqrep            │ arrow_ipc_stream       │ json                    │ session_open  │ {"fields":[{"name":"sql","type":"utf8","nullable":false},{"name":"batch_rows","type":"uint64","nullable":true},{"name":"batch_bytes","type":"uint64","nullable":true}]} │ {"type":"json","session_open":true,"fields":[{"name":"session_id","type":"uint64","nullable":false},{"name":"session_token","type":"string","nullable":false}]} │ false         │ false    │
    │ fetch      │ query   │ Fetch the next Arrow batch from an open session │ reqrep            │ json                   │ arrow_ipc_stream        │ rows          │ {"type":"json","fields":[{"name":"session_id","type":"uint64","nullable":false},{"name":"session_token","type":"string","nullable":false}]}                             │ {"mode":"rows_or_control"}                                                                                                                                      │ false         │ false    │
    │ close      │ query   │ Close an open or exhausted session              │ reqrep            │ json                   │ json                    │ metadata_only │ {"type":"json","fields":[{"name":"session_id","type":"uint64","nullable":false},{"name":"session_token","type":"string","nullable":false}]}                             │ {"type":"json"}                                                                                                                                                 │ false         │ false    │
    │ cancel     │ query   │ Cancel and close an open session                │ reqrep            │ json                   │ json                    │ metadata_only │ {"type":"json","fields":[{"name":"session_id","type":"uint64","nullable":false},{"name":"session_token","type":"string","nullable":false}]}                             │ {"type":"json"}                                                                                                                                                 │ false         │ false    │
    └────────────┴─────────┴─────────────────────────────────────────────────┴───────────────────┴────────────────────────┴─────────────────────────┴───────────────┴─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┴─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┴───────────────┴──────────┘
    ┌────────────────────────────────┐
...
```
    ...
    │ boolean │ varchar │   uint64   │             varchar              │ varchar │   varchar   │              varchar              │
    ├─────────┼─────────┼────────────┼──────────────────────────────────┼─────────┼─────────────┼───────────────────────────────────┤
    │ true    │ NULL    │          1 │ 86f9d36cc1c7317b412898bed0236810 │ closed  │ NULL        │ {"session_id":1,"state":"closed"} │
    └─────────┴─────────┴────────────┴──────────────────────────────────┴─────────┴─────────────┴───────────────────────────────────┘
    ┌─────────────────────────────────────────┐
...
#>   ducknng_start_server('sql_exec', 'ipc:///tmp/ducknng_readme_exec_2ca3374806bae7.ipc', 1, 134217728, 300000, CAST(0 AS "UBIGINT"))
#> 1                                                                                                                              TRUE
