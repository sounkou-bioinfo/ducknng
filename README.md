...
background-job protocol. Query sessions are no longer
bearer-by-`session_id`: `ducknng_open_query(...)` returns a
`session_token` that must accompany `fetch`, `close`, and `cancel`; when
a session is opened over verified mTLS, later session calls must also
arrive with the same transport-derived peer identity. WebSocket here
means NNG’s `ws://` and `wss://` transports, not a second HTTP-specific
RPC surface.
...
```
    ...
    ├─────────┼─────────┼────────────┼──────────────────────────────────┼─────────┼─────────────┼───────────────────────────────────┤
    │ true    │ NULL    │          1 │ 9e83cd06ac950ea7700c50dd824ae741 │ closed  │ NULL        │ {"session_id":1,"state":"closed"} │
    └─────────┴─────────┴────────────┴──────────────────────────────────┴─────────┴─────────────┴───────────────────────────────────┘
    ┌─────────────────────────────────────────┐
...
#>   ducknng_start_server('sql_exec', 'ipc:///tmp/ducknng_readme_exec_2f649a50a48f2c.ipc', 1, 134217728, 300000, CAST(0 AS "UBIGINT"))
#> 1                                                                                                                              TRUE
