...
- Broadened the top-level project framing in the README and package metadata so `ducknng` is no longer described as only REQ/REP interop now that the public socket surface also covers `pair/poly`, `push/pull`, `pub/sub`, and `surveyor/respondent`.
