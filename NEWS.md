...
- Decide whether the current per-session bearer-token ownership model is the sealed session identity contract, and continue toward isolated per-session/per-request DuckDB execution if multi-client state isolation is required.
