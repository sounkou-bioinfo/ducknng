...
- Added the first content-type driven body codec provider layer: `ducknng_list_codecs()`, `ducknng_parse_body(body, content_type)`, and `ducknng_ncurl_table(...)` now expose built-in raw/text/JSON/CSV/TSV/Parquet/Arrow IPC/frame parsing while keeping `ducknng_ncurl(...)` raw and keeping the HTTP server RPC endpoint frame-only. DuckDB-reader-backed codecs use the runtime init-connection execution gate and fail fast inside service-owned SQL until the broader per-session connection model lands. The HTTP frame endpoint now also accepts the frame media type case-insensitively with parameters.
