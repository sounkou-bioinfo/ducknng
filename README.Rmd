...
A tiny local R `servr` server is started during README rendering so this example runs without depending on the public internet. This helper is the low-level HTTP/HTTPS transport primitive; the higher-level RPC and session helpers have not been auto-routed over `http://` and `https://` yet.

```{r http-client-demo-setup, include=FALSE}
