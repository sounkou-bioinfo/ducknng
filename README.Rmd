...
Its SQL-facing client and server interface is explicitly modeled over [`r-lib/nanonext`](https://github.com/r-lib/nanonext): req/rep sockets, dial/listen semantics, context-oriented NNG usage, and a practical messaging surface that can be used directly from DuckDB SQL. Its RPC framing and tabular payload direction are also explicitly informed by Arrow IPC based RPC work such as [`sounkou-bioinfo/mangoro`](https://github.com/sounkou-bioinfo/mangoro) and related projects, where a thin envelope is kept separate from Arrow IPC request and reply payloads.
...
The documented next protocol slice is the session query family: `query_open`, `fetch`, `close`, and `cancel`. In the current docs contract, `query_open` opens one server-owned query session and returns JSON control metadata, `fetch` is the only method that may return streamed row batches, `close` is the normal explicit cleanup path, and `cancel` is best-effort rather than a guarantee of immediate interruption. Until a real owner-identity model is implemented, that family should be treated as scaffolding for loopback or development use rather than production-safe multi-client exposure.
