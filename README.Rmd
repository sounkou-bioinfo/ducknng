...
2. **Framed RPC.** A versioned envelope carries Arrow IPC table payloads or JSON control text. Servers come up with `ducknng_start_server(...)` (NNG) or `ducknng_start_http_server(...)` (HTTP/HTTPS). Clients call by URL; synchronous helpers route by scheme, AIO helpers expose the same calls as futures.
...
Swap `ducknng_start_server(...)` for `ducknng_start_http_server(...)` and the same RPC methods are mounted at an `http://` or `https://` URL path. The **Examples** section walks through transport patterns, AIO, the client surface, body codecs, and sessions in that order.
