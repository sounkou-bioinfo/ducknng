...
- dynamic exact peer-identity and IP/CIDR allowlists through `ducknng_set_tls_peer_allowlist(...)`, `ducknng_set_service_peer_allowlist(...)`, and `ducknng_set_service_ip_allowlist(...)`; NNG services use NNG pipe notifications for new-pipe admission
...
5. optionally update a running service dynamically with `ducknng_set_service_peer_allowlist(...)` and `ducknng_set_service_ip_allowlist(...)`.
