...
- raw socket send/recv primitives plus req-specific raw request/reply helpers
- raw aio helpers for socket send/recv and req-specific request/reply futures
- explicit query-session helpers and TLS config handles

Transport is selected from the URL scheme, so the same SQL-facing operations work across `inproc://`, `ipc://`, `tcp://`, and `tls+tcp://`. The socket layer is explicitly modeled on `nanonext`: open a protocol-specific socket, dial or listen it, and then use raw send/recv or aio helpers as appropriate. The higher-level request helpers remain req-specific, and the session family should still be treated as development scaffolding until multi-client ownership is hardened.
