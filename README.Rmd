...
- first-class raw AIO handles for one pending NNG operation: socket send/recv, HTTP/HTTPS ncurl requests, req-style request/reply futures, and the first raw unary RPC futures
...
- fast C admission checks for mTLS, exact peer-identity allowlists, and compiled IP/CIDR allowlists, plus flexible service-level SQL authorization callbacks through `ducknng_set_service_authorizer(...)` and `ducknng_auth_context()`; NNG services use NNG pipe notifications for new-pipe admission
...
- session ownership by `session_token`, additionally bound to verified mTLS peer identity when present; `query_open` replies expose the server-owned effective `idle_timeout_ms`
...
- envelope-level application authentication is not implemented; the current contract is `session_token` plus optional verified mTLS owner identity
